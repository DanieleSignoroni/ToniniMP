 ...
 * @author AGSOF3
 *
 *	71063	AGSOF3	02/05/2023	Se Ë stata fatta FINE o SOSPENSIONE allora propongo la qta buona prendendola dalla 
 ...
 * 72360    16/02/2026  DSSOF3   Modifiche monitor
 */
...
public class YRilevDatiPrdTSWebFormModifier extends RilevDatiPrdTSWebFormModifier {
	...
	public void displayDichiarazioneInCorso(JspWriter out, RilevDatiPrdTS bo) throws IOException {
		...
		out.println("<td>");
		out.println("<td>");
		out.println("<div class=\"dt-scroll\">");
		out.println("  <table id=\"extraTable\" cellpadding=\"1\" cellspacing=\"3\" class=\"monitorListTable table-container\">"); //Fix 30236
		...
		out.println("    <th class=\"cell\" >" + WebElement.formatStringForHTML(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Qta_UM")) + "</th>");
		out.println("    <th class=\"cell\" >" + WebElement.formatStringForHTML(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Qta_Vrs")) + "</th>");
		out.println("    <th class=\"cell\" >" + WebElement.formatStringForHTML(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Qta_Dsp")) + "</th>");
		...
		out.println("    <th class=\"cell\" >" + WebElement.formatStringForHTML(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "N_ord_bolla")) + "</th>");
		...
		while (iter.hasNext()) {
			...
			if(!testata.getRighe().isEmpty() && isRepartoCongruente(testata, repartoScelto)){
				...
				out.println("<tr " +
						(isInRitardo(testata.getAttivitaEsecutiva()) 
								? "style=\"background-color:"+(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Colore_Ritardo"))+";\"" 
										: "") +
						">");
				...
				BigDecimal[] qtaDaProdurre = RilevDatiPrdTS.calcolaQuantitaDaPorduire(testata.getAttivitaEsecutiva());
				BigDecimal qtaRilevata = testata.getAttivitaEsecutiva().getQtaProdottaUMPrm() != null ? testata.getAttivitaEsecutiva().getQtaProdottaUMPrm() : BigDecimal.ZERO;
				BigDecimal qtaDisponibile = BigDecimal.ZERO;
				if(testata.getAttivitaEsecutiva().isAttivitaIniziale()) {
					qtaDisponibile = qtaDaProdurre[0].subtract(qtaRilevata);
				}else {
					AttivitaEsecutiva prec = testata.getAttivitaEsecutiva().getAtvEsecPrecedente();
					if(prec != null) {
						BigDecimal qtaRilevataFasePrec = prec.getQtaProdottaUMPrm() != null ? prec.getQtaProdottaUMPrm() : BigDecimal.ZERO;
						qtaDisponibile = qtaRilevataFasePrec.subtract(qtaRilevata);
					}
				}
				...
				if (testata.getStatoRilevazione() == RilevazioneDatiProdTes.IN_CORSO) {
					out.println("    <td class=\"cell\" ><button onclick=\"setCurrentEvent(event);sospensioneAction(" + index + ")\" style="+ width +">" + sospensioneBut + "</button><button onclick=\"setCurrentEvent(event);fineAction(" + index + ")\" style=" + width + ">" + fineBut + "</button></td>"); //Fix 24211 //Fix 30236
				...
				AttivitaEsecutiva atv = testata.getAttivitaEsecutiva();
				if(!atv.isAttivitaIniziale()
						&& srcImg.contains("Red.gif")) {
					continue;
				}
				...
				out.println(getHTMLCellaSemaforoQtaProdurreGiacMat(index, testata.getAttivitaEsecutiva(), qtaDisponibile, qtaRilevata));
				out.println("    <td class=\"cell qta-disponibile\" nowrap=\"true\" >" + testata.getAttivitaEsecutiva().getOrdineEsecutivo().getQtaOrdinataUMPrm().intValue() + "</td>");
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + (qtaRilevata != null ? qtaRilevata.intValue() : 0) + "</td>");
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + qtaDisponibile.intValue() +"</td>");
	...
	protected String getHTMLCellaSemaforoQtaProdurreGiacMat(int index, AttivitaEsecutiva attivitaEsecutiva, BigDecimal qtaDisponibile, BigDecimal qtaRilevata) {
		String srcImg = null;
		BigDecimal[] qtaDaProdurre = RilevDatiPrdTS.calcolaQuantitaDaPorduire(attivitaEsecutiva);
		char disp = 'R';
		if(attivitaEsecutiva.isAttivitaIniziale()) {
			disp = calcolaSemaforoQtaProdurreGiacMat(attivitaEsecutiva, qtaDaProdurre[0]);
		}else {
			if(qtaDisponibile != null && qtaDisponibile.compareTo(BigDecimal.ZERO) <= 0) {
				disp = 'R';
			}else if((qtaDisponibile.add(qtaRilevata)).compareTo(attivitaEsecutiva.getOrdineEsecutivo().getQtaOrdinataUMPrm()) >= 0) {
				disp = 'V';
			}else {
				disp = 'G';
			}
	...
	protected String getHTMLCellaQtaGiacMP(int index, AttivitaEsecutiva attivitaEsecutiva) {
		...
		return "<td id=\"QtaGiacMP" + index + "\" class=\"cell\" >" + WebElement.formatStringForHTML(qta != null ? String.valueOf(qta.intValue()) : "0")+"</td>";
	}
	...
	protected BigDecimal giacenzaMaterialePrincipale(AttivitaEsecutiva attivitaEsecutiva) {
		...
		AttivitaEsecMateriale principale = null;
		AttivitaEsecMateriale fallbackManico = null;

		Iterator iterMateriali = attivitaEsecutiva.getMateriali().iterator();
		while (iterMateriali.hasNext()) {
			AttivitaEsecMateriale m = (AttivitaEsecMateriale) iterMateriali.next();
			if (m.getArticolo() == null || m.getArticolo().getIdMacroFamiglia() == null) continue;

			boolean isManico = idMacrofamigliaManici().equals(m.getArticolo().getIdMacroFamiglia());

			if (!isManico) {
				principale = m;              // trovato non-manico => principale
				break;                       // se vuoi "il primo", esci subito
			} else if (fallbackManico == null) {
				fallbackManico = m;          // memorizza il primo manico come fallback
			}
		}
		materiale = (principale != null) ? principale : fallbackManico;
		if(materiale != null) {
			try {
	...
	public int displayProssimeDichInternal(JspWriter out, List prossimeDich, String srcImg, RilevDatiPrdTS bo, int index) throws IOException{ //Fix 23541
		...
		while (iter.hasNext()) {
			...
			if(listaAttivita.getIdAnnoBolla() == null && listaAttivita.getIdNumeroBolla() == null) { //Fix 44763
				AttivitaEsecutiva atv = listaAttivita.getAttivitaEsecutiva();
				if(!atv.isAttivitaIniziale()
						&& srcImg.contains("Red.gif")) {
					continue;
				}
				...
				BigDecimal qtaRilevata = listaAttivita.getAttivitaEsecutiva().getQtaProdottaUMPrm() != null ? listaAttivita.getAttivitaEsecutiva().getQtaProdottaUMPrm() : BigDecimal.ZERO;
				BigDecimal qtaDisponibile = BigDecimal.ZERO;
				if(listaAttivita.getAttivitaEsecutiva().isAttivitaIniziale()) {
					qtaDisponibile = qtaDaProdurre[0].subtract(qtaRilevata);
				}else {
					AttivitaEsecutiva prec = listaAttivita.getAttivitaEsecutiva().getAtvEsecPrecedente();
					if(prec != null) {
						BigDecimal qtaRilevataFasePrec = prec.getQtaProdottaUMPrm() != null ? prec.getQtaProdottaUMPrm() : BigDecimal.ZERO;
						qtaDisponibile = qtaRilevataFasePrec.subtract(qtaRilevata);
					}
				...
				out.println("<tr " +
						(isInRitardo(listaAttivita.getAttivitaEsecutiva()) 
								? "style=\"background-color:"+(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Colore_Ritardo"))+";\"" 
										: "") +
						">");
				...
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + WebElement.formatStringForHTML(idArticolo) + "<br>" + WebElement.formatStringForHTML(descrArticolo) + "</td>");
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + WebElement.formatStringForHTML(articolo_Config) + "<br>" + WebElement.formatStringForHTML(descr_Config) + "</td>");
				out.println(getHTMLCellaSemaforoQtaProdurreGiacMat(index, listaAttivita.getAttivitaEsecutiva(), qtaDisponibile, qtaRilevata));
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + listaAttivita.getAttivitaEsecutiva().getOrdineEsecutivo().getQtaOrdinataUMPrm().intValue() +"</td>");
				out.println("    <td class=\"cell\" nowrap=\"true\" >" + (qtaRilevata != null ? qtaRilevata.intValue() : 0) + "</td>");
				out.println("    <td class=\"cell qta-disponibile\" nowrap=\"true\" >" + qtaDisponibile.intValue() +"</td>");
	...
	protected boolean isInRitardo(AttivitaEsecutiva atv) {
		Date dataRichiesta = getDataInizioRichiesta(atv);
		if (dataRichiesta == null) return false;

		Integer leadTime = Integer.valueOf(ResourceLoader.getString(YRilevDatiPrdTS.YRES_FILE, "Giorni_Ritardo"));
		if (leadTime == null) return false;

		Date oggi = TimeUtils.getCurrentDate();

		Date dataMinimaEvadibile = TimeUtils.addDays(oggi, leadTime);

		return dataRichiesta.before(dataMinimaEvadibile);
	}
	...
	protected BigDecimal ricevaQtaBuonaDaMacchina(RilevDatiPrdTS rilevDatiPrdTS) {
		...
		if(rilevDatiPrdTS.getMacchina() == null)
			return qtaBuona;
		try {
